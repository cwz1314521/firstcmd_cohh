...
public class BatchingTypeManagementController {
     ...
     * 功能描述: 增加修改配料组合列表接口
     *
     * @param boxGroupIngredientCondition 参数对象
     * @return response
     */
    @ApiOperation("增加/修改配料组合")
    ...
    public Response mixAddEdit(@RequestBody @Validated BoxGroupIngredientCondition boxGroupIngredientCondition) {
        Response r = batchingTypeManagementService.binAllocationEditAdd(boxGroupIngredientCondition);
        // modify by zhs at 2018-12-26 start
        if (r.getSuccess()) {
            if (r.getInfo() instanceof Map) {
                Map<String, Object> kafkaMap = (Map<String, Object>) r.getInfo();
                if (null != kafkaMap && !kafkaMap.isEmpty()) {
                    taskKafkaHelper.modifyBoxGroup((Long) kafkaMap.get("groupId"), (List<IngredientBoxBo>) kafkaMap.get("ingredientBoxBos"));
                }
            }
        }
        r.setInfo(null);
        return r;
        // modify by zhs at 2018-12-26 end
    }
