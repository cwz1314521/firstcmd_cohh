...
import com.hema.newretail.backstage.model.data.DrinkAgentStaticsViewBo;
import com.hema.newretail.backstage.model.data.DrinkMachineStaticsViewBo;
import com.hema.newretail.backstage.model.data.ListBo;
...
public class DataUtils {
    ...
    //根据指定的excel模板导出数据
    public static String exportCupExcel(String srcFileName, List<DrinkAgentStaticsViewBo> data, OutputStream fos ,String time) throws Exception {
        //创建Excel文件的输入流对象
        /** InputStream in = DeviceOutStoreExcelUtils.class.getResourceAsStream("/BOOT-INF/classes/excelModel/"+srcFileName);*/
        InputStream in = DeviceOutStoreExcelUtils.class.getResourceAsStream("/excelModel/"+srcFileName);
        //根据模板创建excel工作簿
        ...
        for (int i=0;i<data.size();i++){
            DrinkAgentStaticsViewBo order = data.get(i);
            //创建新的一行，并复制上面的样式
            ...
            newRow.getCell(1).setCellValue(order.getDay());
            newRow.getCell(2).setCellValue(order.getMachineName());
            newRow.getCell(3).setCellValue(order.getAgentName());
            newRow.getCell(4).setCellValue(order.getCupNum());
            newRow.getCell(5).setCellValue(order.getMonthCupNum());
            newRow.getCell(6).setCellValue(order.getTotalCupNum());
            newRow.getCell(7).setCellValue(order.getMaxCupNum());
            newRow.getCell(8).setCellValue(order.getAverageCupNum());
        }
    ...
    //根据指定的excel模板导出数据动态列导出
    public static String exportDataExcel(String srcFileName, ListBo data, OutputStream fos , String time) throws Exception {
        //创建Excel文件的输入流对象
        /** InputStream in = DeviceOutStoreExcelUtils.class.getResourceAsStream("/BOOT-INF/classes/excelModel/"+srcFileName);*/
        InputStream  in = DeviceOutStoreExcelUtils.class.getResourceAsStream("/excelModel/"+srcFileName);
        //根据模板创建excel工作簿
        ...
        TreeSet<String> set = data.getSet();
        List<String> date = data.getDate();
        List<String> see = new ArrayList<> ();
        see.addAll(set);
        for (int e = 1;e < see.size()+1;e++){
            row3.createCell(e).setCellValue(see.get(e-1));
        }
        ...
        for (int i=0;i<data.getMap().size();i++){
            //创建新的一行，并复制上面的样式
            XSSFRow newRow = sheet.createRow(1+i);
            for (int j=0;j<cellStyles.size();j++){
                Cell newCell = newRow.createCell(j);
                newCell.setCellStyle(cellStyles.get(j));
            }
            ...
            newRow.getCell(0).setCellValue(i+1);
            List<Map> map = data.getMap();
            for (int e = 1;e < see.size()+1;e++) {

                if(map.size()<e-1) {
                    Map map1 = map.get(e - 1);
                    if (see.get(e - 1) != null) {
                        if (map1.get(see.get(e - 1)) != null) {
                            newRow.getCell(e).setCellValue(map1.get(see.get(e - 1)).toString());
                        }
                    }
                }
            }
    ...
    //根据指定的excel模板导出数据
    public static String exportDrinkExcel(String srcFileName, List<DrinkMachineStaticsViewBo> data, OutputStream fos , String time) throws Exception {
        //创建Excel文件的输入流对象
        /** InputStream in = DeviceOutStoreExcelUtils.class.getResourceAsStream("/BOOT-INF/classes/excelModel/"+srcFileName);*/
        InputStream in = DeviceOutStoreExcelUtils.class.getResourceAsStream("/excelModel/"+srcFileName);
        //根据模板创建excel工作簿
        ...
        for (int i=0;i<data.size();i++){
            DrinkMachineStaticsViewBo order = data.get(i);
            //创建新的一行，并复制上面的样式
            XSSFRow newRow = sheet.createRow(2+i);
            for (int j=0;j<cellStyles.size();j++){
                Cell newCell = newRow.createCell(j);
            ...
            newRow.getCell(0).setCellValue(i+1);
            newRow.getCell(1).setCellValue(order.getDay());
            newRow.getCell(2).setCellValue(order.getWeek());
            newRow.getCell(3).setCellValue(order.getTotalSaleAmt());
            newRow.getCell(4).setCellValue(order.getTotalRefundAmt());
            newRow.getCell(5).setCellValue(order.getRealIncome());
            newRow.getCell(6).setCellValue(order.getTotalMakeCupNum());
            newRow.getCell(7).setCellValue(order.getTotalCouponNum());
            newRow.getCell(8).setCellValue(order.getTotalUsedCouponNum());
        }
