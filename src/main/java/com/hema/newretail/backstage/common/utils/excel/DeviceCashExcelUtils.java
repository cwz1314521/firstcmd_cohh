...
@Component
public class DeviceCashExcelUtils {
    /**
     *
     * 功能描述: 根据指定的excel模板导出数据
     *
     * @param
     * @return
     * @author  cwz
    ...
    public static void busiComplaintExcel(String orderSrcFileName, List<Map> data, OutputStream fos, String time) throws Exception {
        //创建Excel文件的输入流对象
/**        InputStream in = DeviceCashExcelUtils.class.getResourceAsStream("/BOOT-INF/classes/excelModel/"+orderSrcFileName);*/
        InputStream in = DeviceOutStoreExcelUtils.class.getResourceAsStream("/excelModel/"+orderSrcFileName);
        //根据模板创建excel工作簿
        ...
        for (int i=0;i<data.size();i++){
            ...
            newRow.getCell(1).setCellValue(order.get("userName").toString());
            newRow.getCell(2).setCellValue(order.get("complaintType").toString());
            newRow.getCell(3).setCellValue(order.get("description").toString());
            newRow.getCell(4).setCellValue(order.get("source").toString());
            newRow.getCell(5).setCellValue(order.get("gmtCreate").toString());
            if(order.get("handleStatus").toString().equals("1")){
                newRow.getCell(6).setCellValue("已处理");
            }else {
                newRow.getCell(6).setCellValue("未处理");
            }
            newRow.getCell(7).setCellValue(order.get("handlerName").toString());
            newRow.getCell(8).setCellValue(order.get("remark").toString());
    ...
    public static void exportExcel(String orderSrcFileName, List<CashListBo> data, OutputStream fos, String time) throws Exception {
        //创建Excel文件的输入流对象
/**      InputStream in = DeviceCashExcelUtils.class.getResourceAsStream("/BOOT-INF/classes/excelModel/"+orderSrcFileName);*/
        InputStream in = DeviceOutStoreExcelUtils.class.getResourceAsStream("/excelModel/"+orderSrcFileName);
        //根据模板创建excel工作簿
        ...
        for (int i=0;i<data.size();i++){
            ...
            newRow.getCell(0).setCellValue(i+1);
            newRow.getCell(1).setCellValue(order.getMachineSequence());
            newRow.getCell(2).setCellValue(order.getMachineName());
            newRow.getCell(3).setCellValue(order.getAgentName());
            if(order.getCashPledge() != null){
                newRow.getCell(4).setCellValue(order.getCashPledge().toString());
            }
            if(order.getRetiredCashPledge() != null){
                newRow.getCell(5).setCellValue(order.getRetiredCashPledge().toString());
            }
            if(order.getPenalSum() != null){
                newRow.getCell(6).setCellValue(order.getPenalSum().toString());
            }
            StringBuffer cup = new StringBuffer();
            if(order.getCup() != null && order.getRuleCupMoney() != null){
                cup.append("前").append(order.getCup()).append("杯").append(",").append("每杯返").append(order.getRuleCupMoney()).append("元");
                newRow.getCell(7).setCellValue(cup.toString());
            }
            newRow.getCell(8).setCellValue(order.getGmtModified());
            newRow.getCell(9).setCellValue(order.getOperator());
        }
    ...
    public static void exportAgentAccountExcel(String orderSrcFileName, List<Map> data, OutputStream fos, String time) throws Exception {
        //创建Excel文件的输入流对象
/**        InputStream in = DeviceCashExcelUtils.class.getResourceAsStream("/BOOT-INF/classes/excelModel/"+orderSrcFileName);*/
        InputStream in = DeviceOutStoreExcelUtils.class.getResourceAsStream("/excelModel/"+orderSrcFileName);
        //根据模板创建excel工作簿
        ...
        for (int i=0;i<data.size();i++){
            ...
            newRow.getCell(0).setCellValue(i+1);
            newRow.getCell(1).setCellValue(order.get("id").toString());
            newRow.getCell(2).setCellValue(order.get("accountNumber").toString());
            newRow.getCell(3).setCellValue(order.get("companyName").toString());
            newRow.getCell(4).setCellValue(order.get("earningAmount").toString());
            newRow.getCell(5).setCellValue(order.get("cashAmount").toString());
            newRow.getCell(6).setCellValue(order.get("rawAmount").toString());
            newRow.getCell(7).setCellValue(order.get("freezeAmount").toString());
            newRow.getCell(8).setCellValue(order.get("number").toString());
            newRow.getCell(9).setCellValue(order.get("gmtCreate").toString());
            newRow.getCell(10).setCellValue(order.get("countMachine").toString());
            newRow.getCell(11).setCellValue(order.get("aStatus").toString());
            newRow.getCell(11).setCellValue(order.get("remark").toString());
