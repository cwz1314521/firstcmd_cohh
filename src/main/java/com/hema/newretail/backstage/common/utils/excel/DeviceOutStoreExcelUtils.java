...
import com.hema.newretail.backstage.model.device.outsotre.DeviceOutListExcelBo;
...
@Component
public class DeviceOutStoreExcelUtils {
    /**
     *
     * 功能描述: 根据指定的excel模板导出数据
     *
     * @param deviceOutStoreName
     * @param data
     * @param fos
     * @param time
     * @return null
     ...
     * @date  2018/12/10 10:37
     */
    public static String exportExcel(String deviceOutStoreName, List<DeviceOutListExcelBo> data, OutputStream fos, String time) throws Exception {
        //创建Excel文件的输入流对象
/**      InputStream in = DeviceOutStoreExcelUtils.class.getResourceAsStream("/BOOT-INF/classes/excelModel/"+deviceOutStoreName);*/
        InputStream in = DeviceOutStoreExcelUtils.class.getResourceAsStream("/excelModel/"+deviceOutStoreName);
        //根据模板创建excel工作簿
        ...
        for (int i=0;i<data.size();i++){
            DeviceOutListExcelBo order = data.get(i);
            //创建新的一行，并复制上面的样式
            ...
            newRow.getCell(1).setCellValue(order.getOutstockOrderCode());
            newRow.getCell(2).setCellValue(TimeUtil.getStringByDateFormart(order.getOutstockTime(),"yyyy-MM-dd HH:mm:ss"));
            newRow.getCell(3).setCellValue(order.getAgentCompany());
            newRow.getCell(4).setCellValue(order.getDetails());
            newRow.getCell(5).setCellValue(order.getReceiver());
            newRow.getCell(6).setCellValue(order.getRemark());
        }
