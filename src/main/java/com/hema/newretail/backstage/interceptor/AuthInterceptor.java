        ...
        if (isSessionNull) {
            ...
            if (StringUtils.isEmpty(code)) {
                ...
                if (success) {
                    ...
                    logger.info("获取token失败的redirectUri:" + redirectUri);
                    response.setHeader("redirect_uri", redirectUri);
                    response.setHeader("code_error_code", "304");
                    response.setHeader("Access-Control-Expose-Headers", "redirect_uri,code_error_code");
                    return false;
                }
            }
        } else {
